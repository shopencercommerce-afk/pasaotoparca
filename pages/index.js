    ...
    <SiteLayout>
      <section className="hero">
        <div className="heroText">
          ...
          <p>Marka, model, kategori veya parça kodu ile arayın. Sipariş öncesi stok ve ürün uyumluluğunu WhatsApp üzerinden hızlıca teyit edin.</p>
          <div className="heroActions">
            <a href="/products/" className="primaryBtn">Kataloğu İncele</a>
            <a href={whatsappUrl('Merhaba, ürün uyumluluğu ve stok bilgisi almak istiyorum.')} className="secondaryBtn">WhatsApp’tan Teklif Al</a>
          </div>
        </div>
      ...
      <section className="section">
        ...
        <div className="brandGrid">
          {brands.map(brand => <a className="brandCard" href={productsUrl(brand.slug)} key={brand.slug}><div className="brandLogo">{brandLogoText(brand.name)}</div><strong>{brand.name}</strong><span>{brand.models.join(', ')}</span></a>)}
        </div>
      </section>
      ...
      <section className="section productSection">
        <div className="sectionHead"><div><span>ÖNE ÇIKAN ÜRÜNLER</span><h2>Hızlı teklif alınabilecek ürünler</h2></div><a href="/products/">Kataloğa git</a></div>
          ...
          {featuredProducts.map(product => {
            ...
            return <article className="productCard" key={`${product.no || product.id}-${productCode(product)}`}>
              ...
              <div className="productInfo">
                ...
                <div className="miniBadges"><small>Stok teyidi gerekli</small><small>WhatsApp destek</small></div>
                <strong>{salePrice(product)}</strong>
                <a href={whatsappUrl(`Merhaba, ${product.title} ürünü için teklif almak istiyorum.`)} className="quoteBtn">Teklif Al</a>
              </div>
            </article>
          })}
        </div>
