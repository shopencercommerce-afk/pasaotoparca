    ...
    <main style={{ fontFamily: 'Arial', background: '#0b0b0b', color: '#fff', minHeight: '100vh' }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '22px 40px', borderBottom: '1px solid #222', position: 'sticky', top: 0, backdropFilter: 'blur(12px)', background: 'rgba(10,10,10,0.9)', zIndex: 50 }}>
        ...
        <nav style={{ display: 'flex', gap: '18px', flexWrap: 'wrap', alignItems: 'center' }}>
          {brands.map(brand => <a key={brand.slug} href={productsUrl(brand.slug)} style={{ color: '#ddd', textDecoration: 'none', fontWeight: 600 }}>{brand.name}</a>)}
      ...
      <section style={{ padding: '70px 40px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: '26px' }}>
          {brands.map(brand => (
            <div key={brand.slug} style={{ background: '#151515', border: '1px solid #222', borderRadius: '24px', padding: '30px' }}>
              <a href={productsUrl(brand.slug)} style={{ color: '#fff', textDecoration: 'none' }}><h3 style={{ fontSize: '30px', marginTop: 0 }}>{brand.name}</h3></a>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
                {brand.models.map(model => <a key={model} href={productsUrl(brand.slug, model)} style={{ background: '#202020', padding: '11px 15px', borderRadius: '999px', fontSize: '14px', color: '#fff', textDecoration: 'none' }}>{model}</a>)}
              </div>
            </div>
          ))}
        </div>
