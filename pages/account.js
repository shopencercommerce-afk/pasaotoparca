...
export default function AccountPage() {
  ...
  useEffect(() => {
    ...
    if (token) {
      localStorage.setItem('pasaAuthToken', token)
      ...
      localStorage.setItem('pasaUser', JSON.stringify(googleUser))
  ...
  function logout() {
    ...
    localStorage.removeItem('pasaAuthToken')
    setUser(null)
      ...
      {!user ? (
        <section className="authWrap">
          <div className="authCard">
            <button type="button" className="googleBtn" onClick={googleLogin}><span>G</span> Google ile Giriş Yap</button>
            <div className="divider"><span>veya e-posta ile devam et</span></div>
        ...
        <section className="dashboard">
          ...
          <div className="content">
            ...
            <section id="orders" className="card"><span>SİPARİŞLER</span><h2>Siparişlerim</h2><div className="actionRow"><button onClick={createOrderDraft}>Sepetten Sipariş Taslağı Oluştur</button><a href={`${wpBase}?text=${wpOrderText}`} target="_blank" rel="noreferrer">WhatsApp ile Gönder</a></div>{(user.orders || []).length ? user.orders.map(order => <div className="listItem" key={order.id}><b>{order.id}</b><small>{order.date} • {order.status}</small><p>{order.items?.length || 0} ürün</p></div>) : <div className="empty">Henüz kayıtlı sipariş yok. Sepetteki ürünlerle sipariş taslağı oluşturabilirsiniz.</div>}</section>
