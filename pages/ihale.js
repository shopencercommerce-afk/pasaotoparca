...
export default function IhalePage() {
  ...
  function moneyChange(value, setter) {
    setter(formatInputMoney(value))
  }
      ...
      <section className="cars">
        {cars.length === 0 ? <div className="empty"><h2>Henüz araç kartı yok.</h2><p>İlk aracı ekleyerek maliyet, parça ve satış takibine başlayabilirsin.</p></div> : cars.map(car => {
          ...
          return <article className="car" key={car.id}>
            <div className="carHead"><div><span className={`status ${car.status}`}>{statusLabels[car.status]}</span><h2>{car.title}</h2><p>{[car.brand, car.model, car.plate].filter(Boolean).join(' • ') || 'Araç bilgisi eklenmedi'}</p></div><button onClick={() => deleteCar(car.id)}>Sil</button></div>

            <div className="editCosts"><label>Statü<select value={car.status} onChange={e => updateCar(car.id, { status: e.target.value })}>{Object.entries(statusLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label><label>Alış<input value={car.purchasePrice} onChange={e => moneyChange(e.target.value, value => updateCar(car.id, { purchasePrice: value }))} placeholder="100.000" /></label><label>Çekici<input value={car.towCost} onChange={e => moneyChange(e.target.value, value => updateCar(car.id, { towCost: value }))} placeholder="5.000" /></label><label>Tamir<input value={car.repairCost} onChange={e => moneyChange(e.target.value, value => updateCar(car.id, { repairCost: value }))} placeholder="25.000" /></label><label>Diğer<input value={car.otherCost} onChange={e => moneyChange(e.target.value, value => updateCar(car.id, { otherCost: value }))} placeholder="10.000" /></label><label>Satış<input value={car.salePrice} onChange={e => moneyChange(e.target.value, value => updateCar(car.id, { salePrice: value }))} placeholder="450.000" /></label></div>
