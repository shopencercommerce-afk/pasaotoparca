...
const expenseCategories = [
  { key: 'auctionCommission', label: 'Autogong Komisyon' },
  { key: 'cardCommission', label: 'Kredi Kartı Komisyon' },
  { key: 'notaryCost', label: 'Noter Harç' },
  { key: 'towCost', label: 'Çekici' },
  { key: 'repairCost', label: 'Tamir' },
  { key: 'otherCost', label: 'Kendin Ekle', custom: true }
]
const emptyVehicle = { title: '', brand: '', model: '', plate: '', status: 'gelecek', purchasePrice: '', auctionCommission: '', cardCommission: '', notaryCost: '', towCost: '', repairCost: '', otherCost: '', otherCostLabel: '', salePrice: '', notes: '', neededParts: [], boughtParts: [], tasks: [] }
...
function expenseLabel(item, cat) { return cat.key === 'otherCost' ? (item.otherCostLabel || 'Kendin Ekle') : cat.label }
function expenseList(car) { const item = normalizeCar(car); return expenseCategories.map(cat => ({ ...cat, label: expenseLabel(item, cat), amount: toNumber(item[cat.key]) })).filter(item => item.amount > 0) }
...
function totalCost(car) { const item = normalizeCar(car); const parts = item.boughtParts.reduce((sum, part) => sum + toNumber(part.price), 0); const expenses = expenseCategories.reduce((sum, cat) => sum + toNumber(item[cat.key]), 0); return toNumber(item.purchasePrice) + expenses + parts }
...
export default function IhalePage() {
  ...
  async function persistCar(nextCar, oldCars) {
    setErrorMessage('')
    try {
      const saved = await readJson(await fetch(`/api/ihale/${nextCar.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(nextCar) }))
      setCars(current => current.map(car => car.id === saved.id ? normalizeCar({ ...saved, tasks: nextCar.tasks || car.tasks || [] }) : car))
    } catch (error) { setCars(oldCars); setErrorMessage(error.message || 'Araç güncellenemedi.') }
  }

  async function persistTasks(carId, tasks, oldCars) {
    setErrorMessage('')
    try {
      const savedTasks = await readJson(await fetch('/api/ihale/tasks', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ vehicleId: carId, tasks }) }))
      setCars(current => current.map(car => car.id === carId ? normalizeCar({ ...car, tasks: savedTasks }) : car))
    } catch (error) { setCars(oldCars); setErrorMessage(error.message || 'Görevler kaydedilemedi.') }
  }
  ...
  function updateCar(id, patch) {
    const oldCars = cars
    const nextCars = cars.map(car => car.id === id ? normalizeCar({ ...car, ...patch }) : car)
  ...
  function updateTasks(carId, tasks) {
    const oldCars = cars
    const nextCars = cars.map(car => car.id === carId ? normalizeCar({ ...car, tasks }) : car)
    setCars(nextCars)
  ...
  async function saveCar(e) {
    ...
    setErrorMessage('')
    try {
      const created = await readJson(await fetch('/api/ihale', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...carForm, createdBy: user }) }))
      ...
      setFormOpen(false)
      setView('cars')
    } catch (error) { setErrorMessage(error.message || 'Araç kaydedilemedi.') }
  }

  async function deleteCar(id) {
    if (!confirm('Bu araç kartı silinsin mi?')) return
    const oldCars = cars
    setCars(cars.filter(car => car.id !== id)); setSelectedId('')
    try { await readJson(await fetch(`/api/ihale/${id}`, { method: 'DELETE' })) } catch (error) { setCars(oldCars); setErrorMessage(error.message || 'Araç silinemedi.') }
  }

  function addExpense(carId) {
    const category = partInputs[`expense-category-${carId}`] || 'auctionCommission'
    const amount = partInputs[`expense-amount-${carId}`] || ''
    const customLabel = (partInputs[`expense-custom-${carId}`] || '').trim()
    if (!amount) return alert('Masraf tutarı yazmalısın.')
    if (category === 'otherCost' && !customLabel) return alert('Masraf kategorisini yazmalısın.')
    const car = cars.find(item => item.id === carId)
    if (!car) return
    const patch = { [category]: formatInputMoney(String(toNumber(car[category]) + toNumber(amount))) }
    if (category === 'otherCost') patch.otherCostLabel = customLabel
    updateCar(carId, patch)
    setPartInputs({ ...partInputs, [`expense-amount-${carId}`]: '', [`expense-custom-${carId}`]: '' })
  }

  function clearExpense(carId, category) { updateCar(carId, category === 'otherCost' ? { otherCost: '', otherCostLabel: '' } : { [category]: '' }) }
  ...
  function addNeededPart(carId) {
    ...
    const car = cars.find(item => item.id === carId)
    if (!car) return
    updateCar(carId, { neededParts: [...(car.neededParts || []), { name: value, done: false, addedBy: user }] })
  ...
  function removeNeededPart(carId, partId) { const car = cars.find(item => item.id === carId); if (!car) return; updateCar(carId, { neededParts: (car.neededParts || []).filter(part => part.id !== partId) }) }
  ...
  function addBoughtPart(carId) {
    ...
    const car = cars.find(item => item.id === carId)
    if (!car) return
    updateCar(carId, { boughtParts: [...(car.boughtParts || []), { name, price, buyer }] })
  ...
  function addTask(carId) {
    ...
    const car = cars.find(item => item.id === carId)
    if (!car) return
    updateTasks(carId, [...(car.tasks || []), { title, done: false, assignedTo }])
    setTaskInputs({ ...taskInputs, [`task-title-${carId}`]: '', [`task-user-${carId}`]: user })
  }
  ...
  async function useStockForCar(carId) {
    const stockId = stockInputs[`stock-${carId}`]
    if (!stockId) return alert('Stoktan ürün seçmelisin.')
    const car = cars.find(item => item.id === carId)
    const stock = stockItems.find(item => item.id === stockId)
    if (!car || !stock) return alert('Araç veya stok ürünü bulunamadı.')
    if (Number(stock.quantity || 0) <= 0) return alert('Bu ürün stokta yok.')
    const vehicleText = [car.plate, car.brand, car.model, car.title].filter(Boolean).join(' - ')
    const nextQuantity = Number(stock.quantity || 0) - 1
    const updatedStock = { ...stock, quantity: nextQuantity, note: [stock.note || '', `${new Date().toLocaleString('tr-TR')} - ${vehicleText} için kullanıldı.`].filter(Boolean).join('\n'), status: nextQuantity <= 0 ? 'kullanildi' : stock.status }
    ...
    try {
      ...
      setCars(current => current.map(item => item.id === car.id ? normalizeCar({ ...saved, tasks: car.tasks || [] }) : item))
      setStockInputs({ ...stockInputs, [`stock-${carId}`]: '' })
      alert('Stoktan kullanıldı. Stok adedi düşüldü ve araca işlendi.')
    } catch (error) { setCars(oldCars); setErrorMessage(error.message || 'Stoktan kullanım kaydedilemedi.') }
  }
  ...
  const summary = useMemo(() => {
    ...
    return { totalInvestment, totalSales, soldProfit, byPartner: partners.map(name => ({ name, spend: cars.reduce((sum, car) => sum + partnerSpend(car, name), 0) })), openTasks: allTasks.filter(task => !task.done).length, readyCars: cars.filter(car => car.status === 'hazir').length, repairCars: cars.filter(car => car.status === 'tamirde').length }
  }, [cars, allTasks])
...
function CarModal({ car, user, onClose, updateCar, deleteCar, moneyPatch, addExpense, clearExpense, addNeededPart, toggleNeededPart, removeNeededPart, addBoughtPart, removeBoughtPart, addTask, toggleTask, removeTask, partInputs, setPartInputs, taskInputs, setTaskInputs, stockItems, stockInputs, setStockInputs, useStockForCar }) {
  const cost = totalCost(car)
  const carProfit = profit(car)
  const share = car.status === 'satildi' ? carProfit / partners.length : 0
  const carExpenses = expenseList(car)
  const selectedExpenseCategory = partInputs[`expense-category-${car.id}`] || 'auctionCommission'
  ...
  return <Modal title={car.title} onClose={onClose}>
    ...
    <section className="topDetailGrid"><div className="editMainFields"><label>Statü<select value={car.status} onChange={e => updateCar(car.id, { status: e.target.value })}>{Object.entries(statusLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label><label>Araç Bedeli<input value={car.purchasePrice || ''} onChange={e => moneyPatch(car.id, 'purchasePrice', e.target.value)} placeholder="100.000" /></label><label>Satış<input value={car.salePrice || ''} onChange={e => moneyPatch(car.id, 'salePrice', e.target.value)} placeholder="450.000" /></label></div><div className="expenseBox"><h3>Masraf Ekle</h3><div className="expenseAdd"><select value={selectedExpenseCategory} onChange={e => setPartInputs({ ...partInputs, [`expense-category-${car.id}`]: e.target.value })}>{expenseCategories.map(cat => <option key={cat.key} value={cat.key}>{cat.label}</option>)}</select>{selectedExpenseCategory === 'otherCost' ? <input value={partInputs[`expense-custom-${car.id}`] || ''} onChange={e => setPartInputs({ ...partInputs, [`expense-custom-${car.id}`]: e.target.value })} placeholder="Masraf kategorisi yaz" /> : null}<input value={partInputs[`expense-amount-${car.id}`] || ''} onChange={e => setPartInputs({ ...partInputs, [`expense-amount-${car.id}`]: formatInputMoney(e.target.value) })} placeholder="Tutar" /><button onClick={() => addExpense(car.id)}>Ekle</button></div></div></section>
    <section className="tasksBox importantBox"><h3>Görevler / Yapılacaklar</h3><div className="taskAdd"><input value={taskInputs[`task-title-${car.id}`] || ''} onChange={e => setTaskInputs({ ...taskInputs, [`task-title-${car.id}`]: e.target.value })} placeholder="Örn: Kaportacıya gönder" /><select value={taskInputs[`task-user-${car.id}`] || user} onChange={e => setTaskInputs({ ...taskInputs, [`task-user-${car.id}`]: e.target.value })}>{partners.map(name => <option key={name}>{name}</option>)}</select><button onClick={() => addTask(car.id)}>Görev Ekle</button></div><div className="list">{(car.tasks || []).map((task, index) => <div className="taskRow" key={task.id || index}><label><input type="checkbox" checked={!!task.done} onChange={() => toggleTask(car.id, index)} /><span className={task.done ? 'done' : ''}>{task.title}</span></label><small>{task.assignedTo || '-'}</small><button onClick={() => removeTask(car.id, index)}>×</button></div>)}</div></section>
    <div className="columns importantColumns"><section><h3>Gerekli Parçalar</h3><div className="addLine"><input value={partInputs[`need-${car.id}`] || ''} onChange={e => setPartInputs({ ...partInputs, [`need-${car.id}`]: e.target.value })} placeholder="Örn: Sol far" /><button onClick={() => addNeededPart(car.id)}>Ekle</button></div><div className="list">{(car.neededParts || []).map(part => <div className="listRow" key={part.id}><label><input type="checkbox" checked={!!part.done} onChange={() => toggleNeededPart(car.id, part.id)} /><span className={part.done ? 'done' : ''}>{part.name}</span></label><button onClick={() => removeNeededPart(car.id, part.id)}>×</button></div>)}</div></section><section><h3>Alınan Parçalar</h3><div className="stockUseBox"><select value={stockInputs[`stock-${car.id}`] || ''} onChange={e => setStockInputs({ ...stockInputs, [`stock-${car.id}`]: e.target.value })}><option value="">Stoktan ürün seç</option>{(stockItems || []).filter(item => Number(item.quantity || 0) > 0).map(item => <option key={item.id} value={item.id}>{item.brand} - {item.productName}{item.partCode ? ` - ${item.partCode}` : ''} | Stok: {item.quantity}</option>)}</select><button onClick={() => useStockForCar(car.id)}>Stoktan Kullan</button></div><div className="buyGrid"><input value={partInputs[`bought-name-${car.id}`] || ''} onChange={e => setPartInputs({ ...partInputs, [`bought-name-${car.id}`]: e.target.value })} placeholder="Parça adı" /><input value={partInputs[`bought-price-${car.id}`] || ''} onChange={e => setPartInputs({ ...partInputs, [`bought-price-${car.id}`]: formatInputMoney(e.target.value) })} placeholder="Tutar" /><select value={partInputs[`bought-buyer-${car.id}`] || user} onChange={e => setPartInputs({ ...partInputs, [`bought-buyer-${car.id}`]: e.target.value })}>{partners.map(name => <option key={name}>{name}</option>)}</select><button onClick={() => addBoughtPart(car.id)}>Alındı Ekle</button></div><div className="list">{(car.boughtParts || []).map(part => <div className="boughtRow" key={part.id}><div><b>{part.name}</b><small>{part.buyer || '-'} aldı</small></div><strong>{formatMoney(toNumber(part.price))}</strong><button onClick={() => removeBoughtPart(car.id, part.id)}>×</button></div>)}</div></section></div>
    ...
    <section className="expenseListBox"><h3>Eklenen Masraflar</h3>{carExpenses.length === 0 ? <p>Henüz masraf eklenmedi.</p> : <div className="list">{carExpenses.map(item => <div className="boughtRow" key={item.key}><div><b>{item.label}</b><small>Masraf kategorisi</small></div><strong>{formatMoney(item.amount)}</strong><button onClick={() => clearExpense(car.id, item.key)}>×</button></div>)}</div>}</section>
