...
function completion(car) { const needed = car.neededParts || []; return needed.length ? `${needed.filter(part => part.done).length}/${needed.length} parça` : 'Parça yok' }
function taskCompletion(car) { const tasks = car.tasks || []; return tasks.length ? `${tasks.filter(task => task.done).length}/${tasks.length} görev` : 'Görev yok' }
...
export default function IhalePage() {
  ...
  async function persistCar(nextCar, oldCars) {
    ...
    try {
      ...
      setCars(current =>
        current.map(car =>
          car.id === carId
            ? normalizeCar({ ...car, tasks: savedTasks })
            : car
        )
      )
    } catch (error) {
  ...
  function login(e) {
    ...
    const found = partners.find(
      name =>
        ...
        (normalized === 'omer' && name === 'Ömer')
    )

    if (!found || loginCode !== accessCode)
      return alert('Giriş bilgileri hatalı.')
  ...
  async function saveCar(e) {
    ...
    if (!carForm.title.trim()) {
      return alert('Araç adı yazmalısın.')
    }
  ...
  function removeTask(carId, taskIndex) {
    ...
    const vehicleText = [car.plate, car.brand, car.model, car.title]
      .filter(Boolean)
      .join(' - ')

    const useNote = `${new Date().toLocaleString('tr-TR')} - ${vehicleText} için kullanıldı.`
    const nextQuantity = Number(stock.quantity || 0) - 1

    const updatedStock = {
      ...stock,
      quantity: nextQuantity,
      note: [stock.note || '', useNote].filter(Boolean).join('\n'),
    ...
    try {
      ...
      setStockItems(current =>
        current.map(item =>
          item.id === stock.id ? updatedStock : item
        )
      )
      ...
      setCars(current =>
        current.map(item =>
          item.id === car.id
            ? normalizeCar({ ...saved, tasks: car.tasks || [] })
            : item
        )
      )
  ...
  const summary = useMemo(() => {
    ...
    const soldProfit = cars
      .filter(car => car.status === 'satildi')
      .reduce((sum, car) => sum + profit(car), 0)
    ...
    return (
      <main className="loginPage">
        <section className="loginCard">
          ...
          <p>
            Bu sayfa site içinde görünmez.
            Yetkili ortak girişi gerektirir.
          </p>
          ...
          <small>
            Giriş bilgisi olmayan kişiler panele erişemez.
          </small>
        </section>
  ...
  return (
    <main className="page">
      ...
      <section className="summaryGrid">
        ...
        <button
          className={view === 'cars' ? 'summary active' : 'summary'}
        ...
        <button
          className={view === 'tasks' ? 'summary active' : 'summary'}
      ...
      {/* SADECE HOME EKRANINDA GÖRÜNECEK */}

      {view === 'home' ? (
        <>
          <section className="homeHint">
            <h2>Panel Ana Ekranı</h2>

            <p>
              Araçları görmek için Araçlar kutusuna,
              görevleri görmek için Görevler kutusuna tıkla.
            </p>
          </section>
          ...
          <section className="moneyGrid">
            ...
            <div>
              <span>Satılan Kâr/Zarar</span>

              <strong
                className={
                  summary.soldProfit >= 0
                    ? 'profit'
                    : 'loss'
                }
              >
                {formatMoney(summary.soldProfit)}
              </strong>
            </div>
        ...
        <section className="carsPage">
          ...
          <div className="sectionHead">
            ...
            <div>
              <h2>Araçlar</h2>
              <p>
                Araç kartına tıklayınca detay penceresi açılır.
              </p>
            </div>
          </div>
          ...
          <section className="carsGrid">
            ...
            {cars.length === 0 ? (
              <div className="empty">
                ...
                <p>
                  İlk aracı ekleyerek maliyet, parça ve satış takibine başlayabilirsin.
                </p>
              </div>
            ) : cars.map(rawCar => {
        ...
        <section className="tasksPage">
          ...
          <div className="sectionHead">
            ...
            <div>
              ...
              <p>
                Tüm araçlara ait yapılacak işler burada listelenir.
              </p>
            </div>
          ...
          <section className="tasksPanel">
            ...
            {allTasks.length === 0 ? (
              <div className="empty">
                ...
                <p>
                  Araç detayına girip görev ekleyebilirsin.
                </p>
              </div>
            ) : (
      ...
      {formOpen ? (
        <Modal
          title="Yeni Araç Kartı"
          onClose={() => setFormOpen(false)}
        >

          <form
            onSubmit={saveCar}
            className="carForm simple"
          >
...
function CarModal({
  ...
  return (
    <Modal title={car.title} onClose={onClose}>
      ...
      <div className="editCosts">
        ...
        <label>
          ...
          <input
            ...
            onChange={e =>
              moneyPatch(car.id, 'purchasePrice', e.target.value)
            }
            placeholder="100.000"
          />
        </label>
        ...
        <label>
          Autogong Komisyon
          <input
            value={car.auctionCommission || ''}
            onChange={e =>
              moneyPatch(car.id, 'auctionCommission', e.target.value)
            }
            placeholder="15.000"
          />
        </label>
        ...
        <label>
          ...
          <input
            value={car.cardCommission || ''}
            onChange={e =>
              moneyPatch(car.id, 'cardCommission', e.target.value)
            }
            placeholder="7.500"
          />
        </label>
        ...
        <label>
          Noter Harç
          <input
            value={car.notaryCost || ''}
            onChange={e =>
              moneyPatch(car.id, 'notaryCost', e.target.value)
            }
            placeholder="5.000"
          />
        </label>
        ...
        <label>
          ...
          <input
            value={car.towCost || ''}
            onChange={e =>
              moneyPatch(car.id, 'towCost', e.target.value)
            }
            placeholder="5.000"
          />
        </label>
        ...
        <label>
          ...
          <input
            ...
            onChange={e =>
              moneyPatch(car.id, 'repairCost', e.target.value)
            }
            placeholder="25.000"
          />
        </label>
        ...
        <label>
          ...
          <input
            value={car.otherCost || ''}
            onChange={e =>
              moneyPatch(car.id, 'otherCost', e.target.value)
            }
            placeholder="10.000"
          />
        </label>
        ...
        <label>
          ...
          <input
            value={car.salePrice || ''}
            onChange={e =>
              moneyPatch(car.id, 'salePrice', e.target.value)
            }
            placeholder="450.000"
          />
        </label>
      ...
      <div className="numbers">
        ...
        <div>
          <span>Satış</span>
          <strong>{formatMoney(toNumber(car.salePrice))}</strong>
        </div>

        <div>
          <span>Kâr / Zarar</span>
          <strong className={carProfit >= 0 ? 'profit' : 'loss'}>
            {formatMoney(carProfit)}
          </strong>
        </div>

        <div>
          <span>Kişi Başı Pay</span>
          <strong>
            {car.status === 'satildi' ? formatMoney(share) : '-'}
          </strong>
        </div>
      </div>
      ...
      <div className="columns">
        <section>
          ...
          <div className="addLine">
            <input
              value={partInputs[`need-${car.id}`] || ''}
              onChange={e =>
                setPartInputs({
                  ...partInputs,
                  [`need-${car.id}`]: e.target.value
                })
              }
              placeholder="Örn: Sol far"
            />

            <button onClick={() => addNeededPart(car.id)}>
              Ekle
            </button>
          </div>

          <div className="list">
            {(car.neededParts || []).map(part => (
              <div className="listRow" key={part.id}>
                <label>
                  <input
                    type="checkbox"
                    checked={!!part.done}
                    onChange={() =>
                      toggleNeededPart(car.id, part.id)
                    }
                  />

                  <span className={part.done ? 'done' : ''}>
                    {part.name}
                  </span>
                </label>
                ...
                <button
                  onClick={() =>
                    removeNeededPart(car.id, part.id)
                  }
                >
                  ×
                </button>
        ...
        <section>
          ...
          <div className="stockUseBox">
            <select
              value={stockInputs[`stock-${car.id}`] || ''}
              onChange={e =>
                setStockInputs({
                  ...stockInputs,
                  [`stock-${car.id}`]: e.target.value
                })
              }
            >
            ...
            <button onClick={() => useStockForCar(car.id)}>
              Stoktan Kullan
            </button>
          </div>
          ...
          <div className="buyGrid">
            <input
              value={partInputs[`bought-name-${car.id}`] || ''}
              onChange={e =>
                setPartInputs({
                  ...partInputs,
                  [`bought-name-${car.id}`]: e.target.value
                })
              }
            ...
            <input
              value={partInputs[`bought-price-${car.id}`] || ''}
              onChange={e =>
                setPartInputs({
                  ...partInputs,
                  [`bought-price-${car.id}`]: formatInputMoney(e.target.value)
                })
              }
              placeholder="Tutar"
            />
            ...
            <select
              value={partInputs[`bought-buyer-${car.id}`] || user}
              onChange={e =>
                setPartInputs({
                  ...partInputs,
                  [`bought-buyer-${car.id}`]: e.target.value
                })
              }
            >
              {partners.map(name => (
            ...
            <button onClick={() => addBoughtPart(car.id)}>
              Alındı Ekle
            </button>
          </div>

          <div className="list">
            {(car.boughtParts || []).map(part => (
              <div className="boughtRow" key={part.id}>
                <div>
                  <b>{part.name}</b>
                ...
                <strong>
                  {formatMoney(toNumber(part.price))}
                </strong>

                <button
                  onClick={() =>
                    removeBoughtPart(car.id, part.id)
                  }
                >
                  ×
                </button>
      ...
      <section className="tasksBox">
        ...
        <div className="taskAdd">
          <input
            value={taskInputs[`task-title-${car.id}`] || ''}
            onChange={e =>
              setTaskInputs({
                ...taskInputs,
                [`task-title-${car.id}`]: e.target.value
              })
            }
            placeholder="Örn: Kaportacıya gönder"
          />

          <select
            value={taskInputs[`task-user-${car.id}`] || user}
            onChange={e =>
              setTaskInputs({
                ...taskInputs,
                [`task-user-${car.id}`]: e.target.value
              })
            }
          >
            {partners.map(name => (
        ...
        <div className="list">
          {(car.tasks || []).map((task, index) => (
            <div className="taskRow" key={task.id || index}>
              <label>
                <input
                  type="checkbox"
                  checked={!!task.done}
                  onChange={() => toggleTask(car.id, index)}
                />
              ...
              <button onClick={() => removeTask(car.id, index)}>
                ×
              </button>
            </div>
          ))}
...
.summary,.moneyGrid>div,.partnerGrid>div,.homeHint{background:#fff;color:#151821;border:1px solid #e4e8f1;border-radius:22px;box-shadow:0 16px 55px rgba(23,27,38,.07);padding:16px;text-align:left}
.homeHint{max-width:1360px;margin:0 auto 14px}
.summary{border:0}
...
.carsPage,.tasksPage{max-width:1360px;margin:0 auto}
.sectionHead{display:flex;gap:14px;align-items:center;margin:0 0 14px}
...
.tasksPanel{background:#fff;color:#151821;border-radius:26px;padding:22px;box-shadow:0 18px 60px rgba(23,27,38,.08)}
...
.taskVehicle{display:grid;gap:2px;color:#667085}
.doneTask{opacity:.65}
...
.carForm input,.editCosts input,.editCosts select,.addLine input,.buyGrid input,.buyGrid select,.taskAdd input,.taskAdd select,.stockUseBox select,.notes{border:1px solid #dde2ec;background:#f8f9fc;border-radius:15px;padding:13px;min-width:0;color:#151821}
...
.addLine,.stockUseBox{display:grid;grid-template-columns:1fr auto;gap:8px;margin-bottom:12px}
.addLine button,.buyGrid button,.taskAdd button,.stockUseBox button{border:0;background:#151821;color:#fff;border-radius:14px;padding:0 14px;font-weight:900}
.stockUseBox button{background:#f32334}
...
.moneyGrid,.partnerGrid,.carsGrid,.carForm.simple,.editCosts,.numbers,.columns,.buyGrid,.taskAdd,.stockUseBox{grid-template-columns:1fr}
