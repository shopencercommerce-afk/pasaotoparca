...
function normalizeCar(car) {
  return { ...emptyVehicle, ...car }
}

function totalCost(car) {
  const item = normalizeCar(car)
  const parts = (item.boughtParts || []).reduce((sum, part) => sum + toNumber(part.price), 0)
  return toNumber(item.purchasePrice) + toNumber(item.auctionCommission) + toNumber(item.cardCommission) + toNumber(item.notaryCost) + toNumber(item.towCost) + toNumber(item.repairCost) + toNumber(item.otherCost) + parts
}
...
function partnerSpend(car, name) {
  return (car.boughtParts || []).filter(part => part.buyer === name).reduce((sum, part) => sum + toNumber(part.price), 0)
}

function completion(car) {
  const needed = car.neededParts || []
  if (!needed.length) return 'Parça listesi yok'
  const done = needed.filter(part => part.done).length
  return `${done}/${needed.length} parça tamamlandı`
}
...
export default function IhalePage() {
  ...
  function updateCar(id, patch) {
    setCars(cars.map(car => car.id === id ? normalizeCar({ ...car, ...patch }) : car))
  }
  ...
  return <main className="page">
    ...
    <section className="carsGrid">
      {cars.length === 0 ? <div className="empty"><h2>Henüz araç kartı yok.</h2><p>İlk aracı ekleyerek maliyet, parça ve satış takibine başlayabilirsin.</p></div> : cars.map(rawCar => {
        ...
        const isOpen = expandedId === car.id
        const cost = totalCost(car)
        const carProfit = profit(car)
        const share = car.status === 'satildi' ? carProfit / partners.length : 0
        return <article className={`carCard ${isOpen ? 'open' : ''}`} key={car.id}>
          <button className="cardSummary" onClick={() => setExpandedId(isOpen ? '' : car.id)}>
            ...
            <div className="cardStats"><div><small>Maliyet</small><b>{formatMoney(cost)}</b></div><div><small>Satış</small><b>{formatMoney(toNumber(car.salePrice))}</b></div><div><small>Kâr/Zarar</small><b className={carProfit >= 0 ? 'profit' : 'loss'}>{formatMoney(carProfit)}</b></div></div>
            <em>{completion(car)} • Detay için tıkla</em>
          </button>

          {isOpen ? <div className="detailPanel">
            <div className="detailTop"><div><h3>Araç Detayları</h3><p>Kart içinden maliyetleri, parçaları ve satış durumunu yönet.</p></div><button onClick={() => deleteCar(car.id)}>Aracı Sil</button></div>
