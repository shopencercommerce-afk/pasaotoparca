...
function expenseList(car) { const item = normalizeCar(car); return expenseCategories.map(cat => ({ ...cat, amount: toNumber(item[cat.key]) })).filter(item => item.amount > 0) }
...
export default function IhalePage() {
  ...
  function addExpense(carId) {
    ...
    if (!car) return
    moneyPatch(carId, category, String(toNumber(car[category]) + toNumber(amount)))
    setPartInputs({ ...partInputs, [`expense-amount-${carId}`]: '' })
  }

  function clearExpense(carId, category) { moneyPatch(carId, category, '') }
  ...
  return <main className="page">
    ...
    <section className="summaryGrid">
      <button className={view === 'cars' ? 'summary active' : 'summary'} onClick={() => setView('cars')}><span>🚗 Araçlar</span><strong>{cars.length}</strong><small>Toplam araç</small></button>
      <button className={view === 'tasks' ? 'summary active' : 'summary'} onClick={() => setView('tasks')}><span>📋 Görevler</span><strong>{summary.openTasks}</strong><small>Açık görev</small></button>
      <button className="summary" onClick={() => setView('cars')}><span>🔧 Tamirde</span><strong>{summary.repairCars}</strong><small>Servisteki araç</small></button>
      <button className="summary" onClick={() => setView('cars')}><span>✅ Hazır</span><strong>{summary.readyCars}</strong><small>Satışa hazır</small></button>
    </section>
...
function CarModal({ car, user, onClose, updateCar, deleteCar, moneyPatch, addExpense, clearExpense, addNeededPart, toggleNeededPart, removeNeededPart, addBoughtPart, removeBoughtPart, addTask, toggleTask, removeTask, partInputs, setPartInputs, taskInputs, setTaskInputs, stockItems, stockInputs, setStockInputs, useStockForCar }) {
  ...
  return <Modal title={car.title} onClose={onClose}>
    ...
    <section className="topDetailGrid">
      <div className="editMainFields">
        <label>Statü<select value={car.status} onChange={e => updateCar(car.id, { status: e.target.value })}>{Object.entries(statusLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label>
      ...
      <div className="expenseBox">
        ...
        <div className="expenseAdd">
          <select value={partInputs[`expense-category-${car.id}`] || 'auctionCommission'} onChange={e => setPartInputs({ ...partInputs, [`expense-category-${car.id}`]: e.target.value })}>{expenseCategories.map(cat => <option key={cat.key} value={cat.key}>{cat.label}</option>)}</select>
          <input value={partInputs[`expense-amount-${car.id}`] || ''} onChange={e => setPartInputs({ ...partInputs, [`expense-amount-${car.id}`]: formatInputMoney(e.target.value) })} placeholder="Tutar" />
          <button onClick={() => addExpense(car.id)}>Ekle</button>
        </div>
      </div>
    </section>

    <section className="tasksBox importantBox">
      <h3>Görevler / Yapılacaklar</h3>
      <div className="taskAdd"><input value={taskInputs[`task-title-${car.id}`] || ''} onChange={e => setTaskInputs({ ...taskInputs, [`task-title-${car.id}`]: e.target.value })} placeholder="Örn: Kaportacıya gönder" /><select value={taskInputs[`task-user-${car.id}`] || user} onChange={e => setTaskInputs({ ...taskInputs, [`task-user-${car.id}`]: e.target.value })}>{partners.map(name => <option key={name}>{name}</option>)}</select><button onClick={() => addTask(car.id)}>Görev Ekle</button></div>
      <div className="list">{(car.tasks || []).map((task, index) => <div className="taskRow" key={task.id || index}><label><input type="checkbox" checked={!!task.done} onChange={() => toggleTask(car.id, index)} /><span className={task.done ? 'done' : ''}>{task.title}</span></label><small>{task.assignedTo || '-'}</small><button onClick={() => removeTask(car.id, index)}>×</button></div>)}</div>
    </section>

    <div className="columns importantColumns">
