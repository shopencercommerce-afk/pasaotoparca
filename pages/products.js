  ...
  return (
    <SiteLayout>
      <section className="intro">
        <div><span>ÜRÜN KATALOĞU</span><h1>{pageTitle}</h1><p>{filteredProducts.length} ürün bulundu. Sipariş öncesi stok, fiyat ve uyumluluk WhatsApp üzerinden teyit edilir.</p></div>
        ...
        <div className="catalogMain">
          ...
          <section className="grid">
            {visibleProducts.length === 0 ? <div className="empty"><strong>Ürün bulunamadı.</strong><p>Arama kelimesini sadeleştirin, parça kodunu deneyin veya filtreleri temizleyin.</p><button onClick={() => router.push('/products/', undefined, { shallow: true })}>Filtreleri Temizle</button></div> : visibleProducts.map(product => {
              ...
              const wp = whatsappUrl(`Merhaba, bu ürün için stok ve uyumluluk teyidi almak istiyorum:\n\nÜrün: ${product.title}\nKod: ${productCode(product)}\nFiyat: ${price}\nKategori: ${product.category || 'Diğer'}`)
              return <article key={`${product.no || product.id}-${productCode(product)}`} className="card"><a href={detailUrl} className="cardMain"><div className="imageBox"><img loading="lazy" src={cleanImagePath(product.image)} alt={product.title} /></div><div className="cardBody"><span>{product.category || 'Diğer'}</span><h2>{product.title}</h2><p>Ref: {productCode(product)}</p><div className="badges"><small>Stok teyidi</small><small>Parça kodu kontrolü</small></div><strong>{price}</strong></div></a><div className="buttonRow"><a href={wp} target="_blank" rel="noreferrer" className="wpBtn">Teyit Al</a><button type="button" onClick={() => addToCart(product, setToast)} className="cartBtn">Sepete Ekle</button></div></article>
            })}
          </section>
