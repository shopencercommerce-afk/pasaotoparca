...
const STORAGE_KEY = 'pasa_stok_panel_v1'
...
export default function StokPanel() {
  ...
  useEffect(() => {
    try {
      ...
      console.error('Stok kayıtları kaydedilemedi', error)
    }
  }, [items])
  ...
  function updateSalePrice(id, salePrice) {
    setItems(prev => prev.map(item => item.id === id ? { ...item, salePrice } : item))
  }

  function removeItem(id) {
    if (confirm('Bu stok kaydı silinsin mi?')) setItems(prev => prev.filter(item => item.id !== id))
  }

  function clearAll() {
    if (confirm('Tüm stok kayıtları silinsin mi?')) setItems([])
  }
    ...
    <SiteLayout>
      <section className="stockHero">
        <div>
          ...
          <p>Togg ve Tesla parçalarını; kim aldı, nereden alındı, satış/kullanım durumu ve kâr-zarar hesabı ile takip edin.</p>
        </div>
      ...
      <section className="stockWrap">
        <form className="stockForm" onSubmit={addItem}>
          ...
          <button type="submit">Stoka ekle</button>
        </form>
        ...
        <div className="stockList">
          <div className="listHead">
            <div>
              ...
              <p>{filteredItems.length} kayıt gösteriliyor</p>
            </div>
            <button type="button" className="danger" onClick={clearAll}>Tümünü sil</button>
          </div>
          ...
          <div className="cards">
            {filteredItems.length === 0 && <div className="empty">Henüz kayıt yok. İlk parçayı soldaki formdan ekleyebilirsin.</div>}
